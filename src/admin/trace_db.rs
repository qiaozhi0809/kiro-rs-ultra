...
pub struct TraceQuery {
    ...
    /// 凭据 id 白名单（按分组筛选用）：仅返回 final_credential_id ∈ 列表的 trace。
    /// None = 不过滤；Some(空) = 命中 0 条（分组下没有凭据时的合法值）。
...
impl TraceStore {
    ...
    fn build_where(q: &TraceQuery) -> (String, Vec<Box<dyn rusqlite::ToSql>>) {
        ...
        // 凭据白名单（按分组筛选）。空列表 → "1 = 0" 强制 0 行匹配，
        // 与 stats 接口"分组下没凭据 → 空结果"语义一致。
        if let Some(ids) = &q.credential_ids {
            if ids.is_empty() {
                clauses.push("1 = 0".to_string());
            } else {
                let placeholders = vec!["?"; ids.len()].join(",");
                clauses.push(format!("final_credential_id IN ({})", placeholders));
                for id in ids {
