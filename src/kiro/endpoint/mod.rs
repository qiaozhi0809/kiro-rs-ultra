...
const CLIENT_VALIDATION_REASONS: &[&str] = &["TOOL_USE_RESULT_MISMATCH", "TOOL_DUPLICATE"];
...
mod tests {
    ...
    fn test_default_is_client_validation_error() {
        ...
        // TOOL_DUPLICATE：客户端消息数组里出现重复 tool_use/tool_result id，
        // 生产曾因未识别此 reason 被误判为 502 上游故障，触发下游无限重试风暴。
        assert!(default_is_client_validation_error(
            r#"{"message":"Bedrock error message: The toolResult blocks at messages.4.content contain duplicate Ids: call_x","reason":"TOOL_DUPLICATE"}"#
        ));
